...
export async function GET(context: APIContext) {
	...
	const siteUrl = new URL(url("/"), context.site);
	const atomUrl = new URL(url("/atom.xml"), context.site);
...
<feed xmlns="http://www.w3.org/2005/Atom">
  ...
  <subtitle>${siteConfig.subtitle || "暂无描述"}</subtitle>
	...
	for (const post of posts) {
		...
		for (const anchor of html.querySelectorAll("a")) {
			...
			if (href?.startsWith("/") && !href.startsWith("//")) {
				anchor.setAttribute(
					"href",
					new URL(url(href), context.site).href,
				);
			}
		}
