...
export function url(path: string) {
	// Keep external URLs, protocol-relative URLs, anchors and special schemes intact.
	if (/^(?:[a-z][a-z\d+.-]*:|\/\/|#)/i.test(path)) {
		return path;
	}

	const baseUrl = joinUrl("", import.meta.env.BASE_URL || "/");
	const normalizedPath = joinUrl("", path);
	if (
		baseUrl !== "/" &&
		(normalizedPath === baseUrl.replace(/\/$/, "") ||
			normalizedPath.startsWith(`${baseUrl.replace(/\/$/, "")}/`))
	) {
