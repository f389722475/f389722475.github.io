<script lang="ts">
	...
	// Computed
	const todayYear = $derived(today.getFullYear());
	const todayMonth = $derived(today.getMonth());
	const todayDate = $derived(today.getDate());
	...
	async function fetchCalendarData() {
		try {
			const res = await fetch(url("/api/calendar-data.json"));
			if (!res.ok) {
